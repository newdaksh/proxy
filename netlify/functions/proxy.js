...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-API-Key, X-Forward-Path, x-forward-path',
...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    if (REQUIRED_KEY) {
      const incomingKey = getHeaderCaseInsensitive(event.headers, 'x-api-key') || '';
    ...
    const headerPath = getHeaderCaseInsensitive(event.headers, 'x-forward-path') || '';
    ...
    if ((extraPath === 'tracker' || extraPath === '/tracker') && N8N_TRACKER_URL) {
      upstreamUrl = N8N_TRACKER_URL;
    } else if ((extraPath === 'tracker/suggestions' || extraPath === '/tracker/suggestions') && N8N_TRACKER_SUGGESTIONS_URL) {
    ...
    for (const hn of forwardHeaderNames) {
      const val = getHeaderCaseInsensitive(event.headers, hn) || getHeaderCaseInsensitive(event.headers, hn.toUpperCase());
    ...
    return {
      statusCode: 500,
      headers: corsHeaders,
      body: JSON.stringify({ ok: false, error: 'internal_error', details: err && err.message ? err.message : String(err) }),
    };
  }
};
